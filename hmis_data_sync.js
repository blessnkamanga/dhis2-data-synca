...
var source_username = "administrator";
var source_password = "Testing@2024";
...
function postMohData(
  ...
  var completedataset =
    source_baseurl +
    "/api/completeDataSetRegistrations.json?dataSet=" +
    dataSet +
    "&startDate=" +
    startDate +
    "&endDate=" +
    endDate +
    "&orgUnit=XtF7Xzv3edv&children=true";
  ...
  axios
    ...
    .then((res) => {
      ...
      for (
        var i = 0;
        i < generated_payload.completeDataSetRegistrations.length;
        i++
      ) {
        try {
          //get the organisation unit and the period
          var dataSet =
            generated_payload["completeDataSetRegistrations"][i]["dataSet"];
          var orgunit =
            generated_payload["completeDataSetRegistrations"][i][
              "organisationUnit"
            ];
          var period =
            generated_payload["completeDataSetRegistrations"][i]["period"];
          //get the dataValues
          var datavaluesurl =
            source_baseurl +
            "/api/dataValueSets?dataSet=" +
            dataSet +
            "&orgUnit=" +
            orgunit +
            "&period=" +
          ...
          axios
            ...
            .then((res1) => {
              ...
              var completeDate =
                data_payload["dataValues"][0]["created"].split("T")[0];
              ...
              for (var i = 0; i < data_payload.dataValues.length; i++) {
                ...
                data_payload["dataValues"][i]["attributeOptionCombo"] =
                  "Tt7fU5lUhAU"; //from OHSP to HMIS
